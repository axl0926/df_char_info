...
export default function SearchCharacter() {
    const test = { serverId: "server", characterName: "charactername" };
    const char_info_response = useQuery<{ rows: CharInfo[] }, AxiosError>({ queryKey: ["char_info"], queryFn: async () => (await axios.get(`http://127.0.0.1:8000/df/servers/${test.serverId}/characters?characterName=${test.characterName}`)).data, suspense: true });
